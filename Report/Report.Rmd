...
or_conf_int_plot <- function(mod, subtitle) {
  ...
  p1 <- or_conf_int %>%
    ...
    #   size = 5
    # ) +
    geom_errorbarh(aes(xmin = conf.lower, xmax = conf.upper, y = predictor), color = "#ffa600") +
    geom_point(aes(x = odds.ratio, y = predictor), color = "#003f5c", size = 2) +
...
As you can observe, the AIC for both of these models go down as we clean up the variables and add interactions. Thus, we ended up settling with the third iteration, which are the models with the lowest AIC.
...
However, when evaluating on the same data, we get a relatively high accuracy, which is expected as the model is seeing data that it was modeled on.
