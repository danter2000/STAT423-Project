...
pitcher1 <- glm(wein ~ W + ERA + all_star + WHIP + SO + gold_glove + SV + cy_young_award + most_valuable_player + pitching_triple_crown + 
    rolaids_relief_man_award + nice_guy_awards, family = "binomial", data = PitcherHOF_df)

pitcher2 <- glm(wein ~ W + ERA + all_star + WHIP + SO + gold_glove + SV + cy_young_award + Steroids + most_valuable_player + pitching_triple_crown + rolaids_relief_man_award + nice_guy_awards, family = "binomial", data = PitcherHOF_df)
...
knitr::include_graphics(c("../Plots/corr_pos.png",
                          "../Plots/corr_pitch.png"))
...
```{r results = 'asis'}
extract_eq(final_pos_glm, wrap = TRUE, intercept = "beta", ital_vars = TRUE, use_coefs = TRUE, terms_per_line = 4)
```

```{r results = 'asis'}
extract_eq(final_pitch_glm, wrap = TRUE, intercept = "beta", ital_vars = TRUE, use_coefs = TRUE, terms_per_line = 1)
```
...
* Kai: Mostly worked on model selection for the two models. Also suggested and worked on adding extra variables to the model, such as Steroids and Position and StarterReliever. 
...
Here is the github repository to see our code.

https://github.com/danter2000/STAT423-Project/blob/main/STAT423-Project-Main.Rmd
