...
PositionPlayerHOF_df <- inner_join(batting_df_continuous_counts,
  ...
  mutate(Pos = as.factor(Pos),
         ...
         wein = as.factor(wein)) %>%
  # mutate(wein = as.numeric(as.character(wein))) %>%
...
PredictHOF_pitch_df <- PredictHOF[,c(1,25)] %>%
...
y_hat_train_pos <- predict(good_glm_pos, PredictHOF_pos_df)
y_hat_train_pos[y_hat_train_pos > 0.5] <- 1
train_MSE_pos <- mean((PositionPlayerHOF_df$wein - y_hat_train_pos)^2)

y_hat_test_pos <- predict(good_glm_pos, data = PredictHOF_pos_df)
test_MSE_pos <- mean((PositionPlayerHOF_df$wein - y_hat_test_pos)^2)


# Testing and training mse for pitchers
y_hat_train_pitch <- predict(bad_glm_pitch, data = PitcherHOF_df)
train_MSE_pitch <- mean((PitcherHOF_df$wein - y_hat_train_pitch)^2)

y_hat_test_pitch <- predict(bad_glm_pitch, data = PredictHOF_pitch_df)
test_MSE_pitch <- mean((PitcherHOF_df$wein  - y_hat_test_pitch)^2)
...
```{r dummy data for plot}
data.frame(x = rep(1:10, 20), y = rep(1:20, each = 10)) %>% 
  mutate(type = ifelse(x <= 3 & y == 1, "inducted", ifelse(y <= 5, "eligible", "all"))) %>% 
  write.csv("~/circlestest.csv")

pitcher_WAR <- read.csv("data/pitcherWARupdated.csv")
pos_WAR <- read.csv("data/posplayerWARupdated")

pitcher_WAR %>% 
  mutate(type = "pitcher") %>% 
  left_join(hall_df, by = c("player" = "playerID")) %>% 
  filter(inducted == "Y") %>% 
  rbind(pos_WAR %>% 
  mutate(type = "position") %>% 
  left_join(hall_df, by = c("player" = "playerID")) %>% 
  filter(inducted == "Y")) %>% 
  write.csv("data/vizdata/hof.csv", row.names = F)
```
