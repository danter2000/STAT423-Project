...
hof_players <- hall_df_update %>% 
  ...
  summarize(wein = "Y" %in% inducted * 1, votedBy, yearID, maxYear = max(yearID), yearsOnBallet, fivePercent) %>%
  filter(yearID == maxYear) %>%
  select(-c(yearID, maxYear)) 
...
pos_corr <- get_corr_matrix(PositionPlayerHOF_df[c("G", "AB", "H", "2B", "3B", "HR", "RBI", "SB", "BB", "SO", "IBB", "HBP", "SH", "SF", "GIDP", "AVG", "OBP", "SLG", "WAR")])
replayPlot(pos_corr)

pos_corr <- get_corr_matrix(PitcherHOF_df[c("W", "G", "SV", "ER", "HR", "SO", "IP", "ERA", "WHIP")])
replayPlot(pos_corr)
