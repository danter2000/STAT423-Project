...
Appearances_df <- read_csv(file = "Data/Appearances_2022.csv") 

Appearances_df <- Appearances_df %>%
                  select(1,4) %>%
                  group_by(playerID) %>%
                  mutate(yearID = 1,
                         years_played = sum(yearID)) %>%
                  unique()
...
hof_players <- hall_df %>% 
  filter(category == "Player") %>% 
  group_by(playerID) %>%
  dplyr::summarize(wein = "Y" %in% inducted * 1, votedBy, yearID, maxYear = max(yearID)) %>%
  filter(yearID == maxYear) %>%
  select(-c(yearID, maxYear))

# Unique award winners -- columns represent different rewards
awards_new <- awards_df %>%
  filter(!(yearID > 1933 & awardID == "Baseball Magazine All-Star")) %>%
...
PitcherHOF_df$cy_young_award[is.na(PitcherHOF_df$cy_young_award)] <- 0
PitcherHOF_df$rolaids_relief_man_award[is.na(PitcherHOF_df$rolaids_relief_man_award)] <- 0
PitcherHOF_df$most_valuable_player[is.na(PitcherHOF_df$most_valuable_player)] <- 0
PitcherHOF_df$all_star[is.na(PitcherHOF_df$all_star)] <- 0
...
primary_position <- Appearances %>% 
  ...
  pivot_longer(cols = c(P, C, `1B`, `2B`, `3B`, SS, RF, CF, LF, DH),
               names_to = "Pos") %>%
  group_by(playerID) %>%
  dplyr::summarize(Pos = Pos, Value = value, Max = max(value)) %>%
  filter(Value == Max) %>%
  select(playerID, Pos)
...
PositionPlayerHOF_df$all_star[is.na(PositionPlayerHOF_df$all_star)] <- 0
PositionPlayerHOF_df$most_valuable_player[is.na(PositionPlayerHOF_df$most_valuable_player)] <- 0
PositionPlayerHOF_df$gold_glove[is.na(PositionPlayerHOF_df$gold_glove)] <- 0
PositionPlayerHOF_df$nice_guy_awards[is.na(PositionPlayerHOF_df$nice_guy_awards)] <- 0

row.names(PositionPlayerHOF_df) <- PositionPlayerHOF_df$playerID
```

```{r andy's attempt at doing something useful, warning=FALSE, message=FALSE}
...
hof_players <- hall_df_update %>% 
  ...
  summarize(wein = "Y" %in% inducted * 1, votedBy, yearID,
            maxYear = max(yearID), yearsOnBallet, fivePercent) %>%
  filter(yearID == maxYear) %>%
  select(-c(yearID, maxYear)) 
...
PredictHOF <- player_df %>%
  filter(as.numeric(difftime(finalGame, debut, units="days")) / 365 > 9) %>%
...
PredictHOF_pos_df <- PredictHOF[,c(1,25, 27)] %>%
  ...
  mutate(AVG = round(H / AB, 3), 
         OBP = round((H + BB + HBP) / (AB + ifelse(!is.na(SH), SH, 0) +
         ifelse(!is.na(SF), SF, 0) + BB + HBP), 3),
         `1B` = H - (`2B` + `3B` + HR),
         SLG = round((`1B` + `2B` * 2 + `3B` * 3 + HR * 4) / AB, 3)) %>%
  mutate(Steroids = ifelse(Player %in% espn_ped_df$Player | Player %in% wiki_ped_df$Player, 1, 0)) %>%
  mutate(Steroids = as.factor(Steroids),
  ...
  distinct(playerID, .keep_all = TRUE) %>%
  filter(Pos != "P") %>%
  mutate(most_valuable_player = ifelse(Player == "Paul Goldschmidt",
                                       1, most_valuable_player))
...
PredictHOF_pitch_df <- PredictHOF[,c(1,25, 27)] %>%
  ...
  left_join(Appearances_df[,c(2,3)], by = "playerID") %>%
  mutate(StarterReliever = ifelse((IP / years_played.x) > 100, "Starter", "Reliever")) %>%
  mutate(StarterReliever = as.factor(StarterReliever)) %>%
  mutate(Steroids = ifelse(Player == "Bartolo Colon", 1, Steroids)) %>%
  mutate(cy_young_award = ifelse(Player == "Justin Verlander",
                                 3,
                                 cy_young_award))
...
PositionPlayerHOF_df$Pos <- relevel(as.factor(PositionPlayerHOF_df$Pos),
                                    ref = "DH")
...
bad_glm_pos <- glm(wein ~ G + AB + R + H + HR + RBI + SB + BB +
                                     PrimaryLg + all_star + 
...
good_glm_pos <- glm(wein ~ G + AB + R + H + RBI + SB + BB + PrimaryLg +
...
good_glm_pos_nowar <- step(glm(wein ~ all_star + HR*Steroids + G  +
...
good_glm_pitch <- glm(as.numeric(wein) ~ W + L + G + IP + ERA + all_star + 
                                         WHIP + SO + SV + Steroids +
                                         most_valuable_player + 
                                         pitching_triple_crown + 
                                         nice_guy_awards + all_star +
                                         Steroids:ERA + Steroids:war + 
                                         Steroids:most_valuable_player +
                                         Steroids:SO + votedBy + StarterReliever,
                      data = PitcherHOF_df)

plot(good_glm_pitch, which = 4)

bad_glm_pitch <- glm(as.numeric(wein) ~ W + L + G + IP + ERA + WHIP + SO + SV +
                            ...
                            rolaids_relief_man_award + votedBy + StarterReliever +
                            W:StarterReliever + L:StarterReliever + 
                            SO:StarterReliever,
                      data = PitcherHOF_df)
...
# Step function method of model selection for position players, relevant awards 
ideal_model <- step(good_glm_pos,
                    direction = "both", k = 2)


# Step function method of model selection for position players, all awards 
ideal_model_bad <- step(bad_glm_pos,
                    direction = "both", k = 2)

# Step function method of model selection for pitchers, relevant awards
ideal_model_pitch_good <- step(good_glm_pitch, direction = "both", k = 2)

# Step function method of model selection for pitchers, all awards
ideal_model_pitch_bad <- step(bad_glm_pitch, direction = "both", k = 2)

```

```{r training and test error}

# Testing and training mse for position players
y_hat_train_pos <- predict(final_pos_glm, newdata = PositionPlayerHOF_df, type = "response")
y_hat_train_pos <- ifelse(y_hat_train_pos > 0.5, 1, 0)

y_hat_train_pos_nowar <- predict(good_glm_pos_nowar, newdata = PositionPlayerHOF_df, type = "response")
y_hat_train_pos_nowar <- ifelse(y_hat_train_pos_nowar > 0.5, 1, 0)

y_hat_test_pos <- predict(final_pos_glm, newdata = PredictHOF_pos_df, type = "response")
y_hat_test_pos <- ifelse(y_hat_test_pos > 0.5, 1, 0)

# Adding predicted wein values to the test data sets
PredictHOF_pos_df <- PredictHOF_pos_df %>%
                     mutate(PredictedWein = y_hat_test_pos_nowar)

PredictHOF_pitch_df <- PredictHOF_pitch_df %>%
                     mutate(PredictedWein = y_hat_test_pitch)

y_hat_test_pos_nowar <- predict(final_pos_glm, newdata = PredictHOF_pos_df, type = "response")
y_hat_test_pos_nowar <- ifelse(y_hat_test_pos_nowar > 0.5, 1, 0)

train_MSE_pos <- mean((PositionPlayerHOF_df$wein - y_hat_train_pos)^2)


# Testing and training mse for pitchers
y_hat_train_pitch <- predict(final_pitch_glm, data = PitcherHOF_df, type = "response")
y_hat_train_pitch <- ifelse(y_hat_train_pitch > 0.5, 1, 0)
train_MSE_pitch <- mean((PitcherHOF_df$wein - y_hat_train_pitch)^2)

y_hat_test_pitch <- predict(final_pitch_glm, newdata = PredictHOF_pitch_df, type = "response")
y_hat_test_pitch <- ifelse(y_hat_test_pitch > 0.5, 1, 0)

```

```{r try pitchers}
step(glm(wein ~ G + W + L + WHIP + SV + ERA + IP + R + 
           PrimaryLg + cy_young_award + rolaids_relief_man_award +
                          all_star + most_valuable_player + Steroids +
                           votedBy,
                        data = PitcherHOF_df, family = "binomial"),
                    direction = "both", k = 2)

# The glm object is above for 
...
PositionPlayerHOF_df %>% 
  ggplot() +
  geom_histogram(aes(x = HR, fill = as.factor(wein)))
...
#get_corr_matrix <- function(data) {
#  data[is.na(data)] <- 0
 # res2<-rcorr(as.matrix(as.data.frame(lapply(data, as.numeric))))
#  cor_df <- flattenCorrMatrix(res2$r, res2$P)
#  return(cor_df)
#  cor_df %>%
#    ggplot() +
#    geom_tile(aes(x = row, y = column, fill = cor))
  
#}

get_corr_matrix <- function(data, fp) {
  data[is.na(data)] <- 0
  
  res <- cor(data.frame(lapply(data, as.numeric)))

  p1 <- {
    corrplot(res, method = 'square', order = 'FPC', type = 'lower', tl.col = 'black', tl.srt = 90 , col = COL2('RdBu', 10))
  
    recordPlot()
  }
  
  ggsave(filename = fp, plot=replayPlot(p1))
}


pos_corr <- get_corr_matrix(data = PositionPlayerHOF_df[c("all_star", "SB",
                                                   "HR", "RBI", "Steroids",
                                                   "nice_guy_awards", "AVG", "R",
                                                   "most_valuable_player")],
                            fp = "../Plots/corr_pos_new.png")
replayPlot(pos_corr)

pitch_corr <- get_corr_matrix(data = PitcherHOF_df[c("W", "SO", "Steroids",
                                            "most_valuable_player","all_star",
                                            "cy_young_award",
                                            )],
                              fp = "Plots/corr_pitch_new.png")
replayPlot(pitch_corr)


#ggsave(filename = "STAT423-Project/Plots/corr_pos_new.png", (pos_corr))

#ggsave(filename = "../Plots/corr_pitch_new.png", replayPlot(pitch_corr))
```

```{r awards sanity check}
PositionPlayerHOF_df[floor(runif(10, 1, nrow(PositionPlayerHOF_df))),] %>% 
  select(playerID, Player, all_star)

batting_df %>% 
  group_by(playerID) %>% 
  summarize(seasons = n()) %>% 
  filter(seasons >= 10) %>% 
  write.csv("data/hof_eligible_pos.csv")

pitching_df %>% 
  group_by(playerID) %>% 
  summarize(seasons = n()) %>% 
  filter(seasons >= 10) %>% 
  write.csv("data/hof_eligible_pitcher.csv")
```
...
summary(final_pos_glm)
saveRDS(final_pos_glm, "data/final_pos_glm.RDS")

plot(final_pos_glm, which = 4)

# Pitcher
row.names(PitcherHOF_df) <- PitcherHOF_df$Player
...
summary(final_pitch_glm)
saveRDS(final_pitch_glm, "data/final_pitch_glm.RDS")
...
```{r}
pos_fitted <- fitted(final_pos_glm)

confusion_pos <- PositionPlayerHOF_df %>%
  filter(!is.na(RBI)) %>%
  filter(!is.na(SB)) %>%
  mutate(i = 1:n()) %>% 
  left_join(data.frame(i = 1:length(pos_fitted), fit = pos_fitted), 
            by = "i") %>% 
  select(-i) %>% 
...
misclassfication_pos <- sum(ifelse(confusion_pos$wein_pred != confusion_pos$wein, confusion_pos$n, 0)) / sum(confusion_pos$n)

pitch_fitted <- fitted(final_pitch_glm)
confusion_pitch <- PitcherHOF_df %>%
  mutate(i = 1:n()) %>%
  left_join(data.frame(i = 1:length(pitch_fitted), fit = pitch_fitted),
            by = "i") %>%
  select(-i) %>%
  mutate(wein_pred = as.factor((fit > 0.5)*1)) %>% 
  group_by(wein_pred, wein) %>% 
...
misclassfication_pitch <- sum(ifelse(confusion_pitch$wein_pred != confusion_pitch$wein, confusion_pitch$n, 0)) / sum(confusion_pitch$n)
...
get_confusion_matrix(final_glm_pos, PositionPlayerHOF_df)
```
