...
PositionPlayerHOF_df <- inner_join(batting_df_continuous_counts,
                        ...
                        mutate(across(c(19, 20, 23, 29, 41), as.factor))
...
ggplot(data = PositionPlayerHOF_df, mapping = aes(x = wein, y = HR)) + 
  geom_boxplot()

# Boxplot comparing hit totals in non-hall of famers to hall of famers
ggplot(data = PositionPlayerHOF_df, mapping = aes(x = wein, y = H)) + 
  geom_boxplot()

# Boxplots stratified by number of mvp's, grouped by hof, on number of hr totals 
ggplot(data = PositionPlayerHOF_df, mapping = aes(x = wein, y = HR, color = most_valuable_player)) + 
  geom_boxplot()
