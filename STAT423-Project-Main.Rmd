...
PitcherHOF_df <- inner_join(pitching_df_continuous_counts,
  ...
  mutate(votedBy = ifelse(Player == "Charles Bender", "Veterans",
                   ...
                   ifelse(Player == "Bill Donovan", "Veterans",
                          votedBy))))))))))) %>%
  mutate(all_star = all_star + as.numeric(baseball_magazine_all_star)) %>%
...
y_hat_train_pos <- predict(good_glm_pos, newdata = PositionPlayerHOF_df, type = "response")
...
y_hat_train_pitch <- predict(bad_glm_pitch, data = PitcherHOF_df, type = "response")
...
```{r try pitchers}
step(glm(wein ~ G + WHIP + SV + ERA + IP + R + 
           PrimaryLg + cy_young_award + tsn_reliever_of_the_year + 
                          all_star*WAR + most_valuable_player + WAR:Steroids +
                          nice_guy_awards + votedBy,
...
final_pos_glm <- glm(wein ~ all_star + HR + Steroids + Pos + votedBy + RBI + R +
                       SB + HR:Steroids + Steroids:SLG,
                     family = "binomial",
                     data = PositionPlayerHOF_df)

summary(final_pos_glm)

final_pitch_glm <- glm(wein ~ W + L + all_star + SO + SV + as.factor(Steroids) +
                              rolaids_relief_man_award + votedBy,
                       family = "binomial",
                       data = PitcherHOF_df)
...
```{r}
pos_fitted <- fitted(good_glm_pos)

PositionPlayerHOF_df %>% 
  mutate(i = 1:n()) %>% 
  left_join(data.frame(i = as.numeric(names(pos_fitted)), fit = pos_fitted), by = "i") %>% 
  select(-i) %>% 
...
library(DHARMa)
plot(good_glm_pos)
testDispersion(good_glm_pos, plot=T,
type="PearsonChisq",
alternative="less")
```
