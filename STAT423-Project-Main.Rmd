...
PositionPlayerHOF_df <- inner_join(batting_df_continuous_counts,
  ...
  inner_join(awards_wider, by = "playerID") %>%
  inner_join(hof_players, by = "playerID") %>%
  subset(select = c(1:26, 28:31, 34:37, 40:42, 44, 45, 47, 49)) %>%
  ...
  left_join(primary_position, by = "playerID")
...
# Pitcher Model selection BAD MODEL (GG, ROLOAIDS INCLUDED) ----
...
lm_pitcher_subset_bad <- regsubsets(as.factor(wein) ~ W + L + G + IP + ERA +
                                                      ...
                                                      cy_young_award +
                                                      Steroids +
                                                      as.factor(most_valuable_player) + 
...
# Pitcher Model SELECTION GOOD (ONLY COUNTING, RATE, & RELEVANT AWARDS) ----
...
# Positon Player Model selection ----

# Trying to find best 7 predictors
lm_position_subset <- regsubsets(as.factor(wein) ~ G + AB + R + H + HR + RBI +
                                                   SB + BB + PrimaryLg +
                                                   all_star +
                                                   as.factor(most_valuable_player) +
                                                   AVG + OBP + SLG + Steroids +
                                                   nice_guy_awards + WAR + Pos,
                                 data = PositionPlayerHOF_df)
summary(lm_position_subset)
